...
;;
;; Returns true if list contains s
(check-expect (contains? "UBC" empty) false)
(check-expect (contains? "UBC" (cons "UBC" empty)) true)
(check-expect (contains? "UBC" (cons "McGill" empty)) false)
...
;; Takes fn and (list n0 n1 ...) and returns (list (fn n0) (fn n1) ...)
(check-expect (map2 sqr empty) empty)
(check-expect (map2 sqr (list 2 4)) (list 4 16))
(check-expect (map2 sqrt (list 4 16)) (list 2 4))
...
;; Takes a predicate and list and returns a list that only includes values where the predicate function would return true 
